...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i < tiles; i++){
        for (int j = 0; j < tiles; j++){
            // ランダム
            int rannum =arc4random() % color_num;     // rannum = {0,1,2,3,4}
...
- (IBAction)pushedRed:(id)sender {
    ...
    _limitNum.text = [NSString stringWithFormat:@"step  %d / 10",_limit];
}
...
- (void)gameFinished {
    ...
    //ゲームオーバー画面
        ...
        failed.font = [UIFont systemFontOfSize:28];
